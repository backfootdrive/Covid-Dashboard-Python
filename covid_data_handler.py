...
def schedule_covid_updates(update_interval: int,update_name, scheduler,config):
...
def covid_api_request(location='Exeter',location_type='ltla',nation='England') -> dict:
    ...
    try:
        ...
        national_data = api.get_json(save_as='data\\national_data.json')
    ...
    try:
        ...
        local_data = api.get_json(save_as='data\\local_data.json')
    except exceptions.FailedRequestError as inst:
        x = str(inst.args[0]).split('\n')
