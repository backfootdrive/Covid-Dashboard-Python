...
def template():
    ...
    local_7day_cases = data.process_covid_json_data(data.data['local data'],'local')
    national_7day_cases,total_deaths,hospital_cases = data.process_covid_json_data(data.data['national data'],'national')
...
def update_data(x: int,update_interval: int,s):
    ...
    if x >= 2:
        ...
        s, e2 = news.update_news(news.news_API_request,update_interval+1,s,config['news'])
        logger.info('Update Scheduled Successfully!')
        events.append(e2)
    return s, events
